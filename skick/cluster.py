...
class ClusterDataManager:
    ...
    def delete_service(self, address):
        ...
        for service in self.local_services:
            if address in self.local_services[service]:
                self.local_services[service].remove(address)
                self.infos.add_information({"key": {"shard": self.shard,
                                                    "service": service,
                                                    "address": address},
                                            "timestamp": monotonic(),
                                            "info": False})
    ...
    def update_bucket(self, kill=False):
        ...
        timestamp = monotonic()
        for service in self.local_services:
            for provider in self.local_services[service]:
                self.infos.add_information({"key": {"shard": self.shard,
                                                    "service": service,
                                                    "address": provider},
                                            "timestamp": timestamp,
    ...
    def decode_bucket(self):
        ...
        self.remote_services = self.infos.extract(filter_func=filter_remote_services,
        ...
        shards = set(self.infos.extract(filter_func=filter_shards,
                                        map_func=map_shard,
                                        to_dict=False))
        ...
        self.remote_shards = self.infos.extract(filter_func=filter_shard_services,
        ...
        self.remote_factories = self.infos.extract(filter_func=filter_factories,
        ...
        corrections = self.infos.extract(filter_func=filter_erroneous_locals,
    ...
    def kill_shard(self, shard):
        """
        Kills a remote shard that has been found to be unresponsive. We need to
        filter out the appropriate information from the bucket. And flip it to
        indicate the death of the actor.      
        """
        timestamp = monotonic()
        ...
        corrections = self.infos.extract(filter_func=filter_shard,
                                         map_func=map_shard,
                                         to_dict=False)
        self.infos.consume_batch(corrections)
    ...
    def make_consistent(self):
        ...
        consistetnt. Namely, it will attempt to detect living services and
        ...
        def map_dead_shards(info):
            ...
            key, _ = info
            return key["shard"]
        dead_shards = set(self.infos.extract(filter_func=filter_dead_shards,
                                             map_func = map_dead_shards,
                                             to_dict=False))
        ...
        def map_inconsistent(info):
            ...
            key, (_, alive) = info
            return {"key": key, "timestamp": timestamp, "info": False}
        corrections = self.infos.extract(filter_func=filter_inconsistent,
                                         map_func=map_inconsistent,
                                         to_dict=False)
...
cluster_data = ClusterDataManager()