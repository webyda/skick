...
because without it, starting new actors is too time consuming.
"""
...
class RoutedMessage(MessageSystemInterface):
    ...
    async def mailman(self, actor):
        await self.simple_interface.mailman(actor)
    ...
    async def send(self, address, message):
        ...
        system. Otherwise, it will use the RabbitMessage system to send the
    ...
    async def register_shard(self, address):
        ...
        self.factory.queue = await self.factory.out_channel.declare_queue(shard, auto_delete=True)
...
class RoutedFactory(MessageSystemFactory):
    ...
    async def connect(self, config):
        ...
        self.in_exchange = await self.in_channel.declare_exchange("ActorExchange", "direct")
        self.out_exchange = await self.out_channel.declare_exchange("ActorExchange", "direct")
        self.out_broadcast = await self.out_channel.declare_exchange("BroadcastExchange", "fanout")
        self.in_broadcast = await self.in_channel.declare_exchange("BroadcastExchange", "fanout")
        
        self.future.set_result((self.in_connection,
                                self.out_connection,
                                self.in_channel,
                                self.out_channel,
                                self.out_exchange,
